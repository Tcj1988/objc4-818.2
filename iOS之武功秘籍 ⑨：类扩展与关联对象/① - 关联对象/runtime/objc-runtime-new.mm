...
addClassTableEntry(Class cls, bool addMeta = true)
{
    ...
    //allocatedClasses 开辟的类的表,在objc_init中的runtime_init就初始化创建了
    auto &set = objc::allocatedClasses.get();
    ...
    if (!isKnownClass(cls))
        set.insert(cls);
    if (addMeta)
        addClassTableEntry(cls->ISA(), false);//添加到allocatedClasses哈希表
}
...
public:
    ...
    void attachToClass(Class cls, Class previously, int flags)
    {
        ...
        if (strcmp(mangledName, TCJPersonName) == 0) {
            bool cj_isMeta = cls->isMetaClass();
            auto cj_rw = cls->data();
            auto cj_ro = cj_rw->ro();
            if (!cj_isMeta) {
                printf("%s 这个是我要研究的 %s \n",__func__,TCJPersonName);
            }
        }
        ...
        if (it != map.end()) {
            ...
            } else {//如果不是元类，则只走一次 attachCategories
                 ...
                 int flags)
{
    ...
    for (uint32_t i = 0; i < cats_count; i++) {
        ...
        if (mlist) {
            //mcount = 0，ATTACH_BUFSIZ= 64，不会走到if里面的流程
...
static Class realizeClassWithoutSwift(Class cls, Class previously)
{
    ...
    //读取类结构的bits属性、//ro -- clean memory，在编译时就已经确定了内存
    ...
    if (ro->flags & RO_FUTURE) {
        ...
        rw->flags = RW_REALIZED|RW_REALIZING|isMeta;//将class的data重新复制为rw
    ...
    /*
     ...
     实现元类、父类
     当isa找到根元类之后，根元类的isa是指向自己的，不会返回nil从而导致死循环
     ——remapClass中对类在表中进行查找的操作，如果表中已有该类，则返回一个空值；
...
load_images(const char *path __unused, const struct mach_header *mh)
{
    ...
    // Discover load methods 发现加载load方法
    {
        mutex_locker_t lock2(runtimeLock);
        prepare_load_methods((const headerType *)mh);
    }

    // Call +load methods (without runtimeLock - re-entrant)调用load
    call_load_methods();
}
...
{
    recursive_mutex_locker_t lock(loadMethodLock);//对方法加载上锁(递归锁)
    mutex_locker_t lock2(runtimeLock);//对runtime上锁(互斥锁)
    unmap_image_nolock(mh);//1._unload_image(hi):卸载hi的数据 2.removeHeader(hi):移除hi数据
}
...
static void schedule_class_load(Class cls)
{
    ...
    schedule_class_load(cls->getSuperclass());//递归父类,直到cls不存在
    //将所有有+load方法的类跟+load方法加入到loadable_classes表
...
void prepare_load_methods(const headerType *mhdr)
{
    ...
    for (i = 0; i < count; i++) {
        //遍历这些类,并将其+load方法添加到loadable_classes数组中保存,优先添加其父类的+load方法
        schedule_class_load(remapClass(classlist[i]));
    }
    //获取所有非懒加载分类
    category_t * const *categorylist = _getObjc2NonlazyCategoryList(mhdr, &count);
    for (i = 0; i < count; i++) {//遍历这些分类
        category_t *cat = categorylist[i];
        Class cls = remapClass(cat->cls);
        if (!cls) continue;  // category for ignored weak-linked class 如果没有找到分类所属的类就到此为止,处理数组中的下一个分类
        if (cls->isSwiftStable()) {
            _objc_fatal("Swift class extensions and categories on Swift "
                        "classes are not allowed to have +load methods");
        }
        realizeClassWithoutSwift(cls, nil);//如果分类所属的类没有实现就先去实现
        ASSERT(cls->ISA()->isRealized());
        //遍历这些分类,并将其+load方法添加到loadable_categories数组中保存
...
objc_class::getLoadMethod()
{
    ...
    mlist = ISA()->data()->ro()->baseMethods();//类的方法列表
    if (mlist) { //方法列表中查找+load方法
        for (const auto& meth : *mlist) {
            const char *name = sel_cname(meth.name());
...
findMethodInSortedMethodList(SEL key, const method_list_t *list, const getNameFunc &getName)
{
    ...
    uintptr_t keyValue = (uintptr_t)key; //key 等于 sayNB
    uint32_t count;
...
resolveMethod_locked(id inst, SEL sel, Class cls, int behavior)
{
    ...
    if (! cls->isMetaClass()) { //类不是元类，调用对象的解析方法
        ...
        if (!lookUpImpOrNilTryCache(inst, sel, cls)) {//如果没有找到或者为空，在元类的对象方法解析方法中查找
            resolveInstanceMethod(inst, sel, cls);
        }
    }
