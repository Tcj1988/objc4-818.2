...
public:
  ...
  iterator find(const_arg_type_t<KeyT> Val) {
    BucketT *TheBucket;
    if (LookupBucketFor(Val, TheBucket)) //查找桶子
      return makeIterator(TheBucket, getBucketsEnd(), true);//如果找到则组合桶子返回
    return end();//结束迭代
  }
  ...
  std::pair<iterator, bool> try_emplace(const KeyT &Key, Ts &&... Args) {
    BucketT *TheBucket;
    if (LookupBucketFor(Key, TheBucket)) //找桶子.其中key是关联对象
      return std::make_pair( //如果桶子存在,则返回,置为false的原因是哈希map中已经存在
               makeIterator(TheBucket, getBucketsEnd(), true),
               false); // Already in map. 已经存在,直接返回false

    // Otherwise, insert the new element.
    TheBucket = InsertIntoBucket(TheBucket, Key, std::forward<Ts>(Args)...);//往桶里添加值
    return std::make_pair(
             makeIterator(TheBucket, getBucketsEnd(), true),
             true);//bool值置为true,表示第一次往哈希map添加桶子
  }
...
private:
  ...
  bool LookupBucketFor(const LookupKeyT &Val,
    ...
    unsigned BucketNo = getHashValue(Val) & (NumBuckets-1);//通过哈希函数得到下标
    unsigned ProbeAmt = 1;
    while (true) { //通过 哈希查找
  ...
  template <typename LookupKeyT>//重载函数,在try_emplace中调用的是这个函数
