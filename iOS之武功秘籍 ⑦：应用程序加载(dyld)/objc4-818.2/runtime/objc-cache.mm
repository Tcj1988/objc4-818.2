...
static inline mask_t cache_hash(SEL sel, mask_t mask) 
{
    ...
    value ^= value >> 7;
#endif
    return (mask_t)(value & mask);// 通过sel & mask（mask = cap -1）
}
...
{
    bucket_t *oldBuckets = buckets();//判断是否有旧的bucket,第一次肯定是什么旧的bucket的
    bucket_t *newBuckets = allocateBuckets(newCapacity);//开辟bucket,此时只是一个临时变量
    ...
    setBucketsAndMask(newBuckets, newCapacity - 1);//将临时的bucket存入缓存中
...
void cache_t::insert(SEL sel, IMP imp, id receiver)
{
    ...
    mask_t newOccupied = occupied() + 1;//没有属性赋值的情况下 occupied() = 0, newOccupied = 1
    unsigned oldCapacity = capacity(), capacity = oldCapacity;
    if (slowpath(isConstantEmptyCache())) {//小概率发生的,即当 occupied() = 0时,即创建缓存,创建属于小概率事件
        // Cache is read-only. Replace it.
        if (!capacity) capacity = INIT_CACHE_SIZE;//初始化时,capacity = 4 (1<<2)
        ...
        /*第一次时,申请开辟的内存是4个,如果此时已经有3个从bucket插入到cache里面,再插入一个,就是4个.
         当大于4即当前下标是4,就数组越界了,这是肯定不行的,所以需要在原理的容量上进行两倍扩容
         */
    }
...
#endif
    else {//如果超出了3/4,则需要扩容(两倍扩容) -- 即 occupied 为2时,就没有进去了
        //扩容算法: 有capacity时,扩容两倍,没有capacity就初始化为4
    ...
    mask_t begin = cache_hash(sel, m);//求cache哈希,即哈希下标---通过哈希算法函数计算 sel存储的下标
    mask_t i = begin;
    ...
    do {
        ...
        if (fastpath(b[i].sel() == 0)) {
            //则将sel存储进去,并将对应的 occupied标记++,即从0变为1
            incrementOccupied();
        ...
        //如果当前计算的哈希下标已经存储了sel,且两个sel不相等,需要重新进行哈希计算 得到新的下标
...
static void _garbage_make_room(void)
{
    ...
    // Create the collection table the first time it is needed 第一次需要时创建收集表
    if (first)
    {
        first = 0;
    ...
    // Double the table if it is full 内存段加大了 原有内存*2,capacity扩容了两倍,实际的内存也需要扩容两倍
...
void cache_t::collect_free(bucket_t *data, mask_t capacity)
{
    ...
    cache_t::collectNolock(false);//垃圾回收,清理旧的bucket
}
