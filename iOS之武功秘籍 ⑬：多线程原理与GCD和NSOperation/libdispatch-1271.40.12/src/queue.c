		...
		dispatch_object_t dou, dispatch_qos_t qos)
{
	if (likely(!_dispatch_object_is_redirection(dou))) {
		...
		// this continuation. 查找具有已设置自动释放频率的降序目标顺序的第一个队列,并将其用作此继续的频率
		dou._dc->dc_ctxt = (void *) //对ctxt进行处理
		(uintptr_t)_dispatch_queue_autorelease_frequency(dl);
	}

	dispatch_queue_t dq = dl->do_targetq;//拿到dl的target,做了一些处理
	if (!qos) qos = _dispatch_priority_qos(dq->dq_priority);
	dx_push(dq, dou, qos);//递归 队列也是一个对象, 有父类 根类
}
...
{
	_dispatch_sync_function_invoke_inline(dq, ctxt, func);//下层的调用执行
	_dispatch_trace_item_complete(dc);
	...
	// similar to _dispatch_queue_drain_try_unlock 对下层进行状态的释放
...
{
	uint64_t dq_state = _dispatch_wait_prepare(dq);//判断dq是否为正在等待的主队列
...
#if DISPATCH_USE_KEVENT_WORKLOOP
	} else if (wq_supported & WORKQ_FEATURE_WORKLOOP) { //任务循环
#if DISPATCH_USE_KEVENT_SETUP
		cfg.workq_cb = _dispatch_worker_thread2;
...
#endif // DISPATCH_USE_KEVENT_WORKLOOP
#if DISPATCH_USE_KEVENT_WORKQUEUE
	} else if (wq_supported & WORKQ_FEATURE_KEVENT) { //内核事务
#if DISPATCH_USE_KEVENT_SETUP
		cfg.workq_cb = _dispatch_worker_thread2;
		cfg.kevent_cb = (pthread_workqueue_function_kevent_t) _dispatch_kevent_worker_thread;
		r = pthread_workqueue_setup(&cfg, sizeof(cfg));
